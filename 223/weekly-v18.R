                            ...
                            error=c('RMSE'), step_ahead=1, start=1)
{
  ...
  fcst_af <- forecast(bestfit, xreg = fourier(train, K=k, h=26))                    #2
  ...
  fcst_stle <- try(stlf(train, h=26, method = "ets"),silent = TRUE)      #5
  ...
  avg <- mean(train[(length(train)-26):length(train)]) #8
  fcst_avg <- rep(avg,26)
  error_avg <- smape_cal(fcst_avg,test)
  
  fcst_naive <- naive(train, h=26)     #9
  error_naive <- smape_cal(fcst_naive$mean,test) 
  
  fcst_ses1 <- ses(train, h=26, alpha = .3) #10
  error_ses1 <- smape_cal(fcst_ses1$mean,test) 
  
  fcst_ses2 <- ses(train, h=26, alpha = .1) #11
  error_ses2 <- smape_cal(fcst_ses2$mean,test) 
  
  fcst_ses3 <- ses(train, h=26) #12
  error_ses3 <- smape_cal(fcst_ses3$mean,test) 
  
  fcst_holt <- holt(train, h=26) #13
  error_holt <- smape_cal(fcst_holt$mean,test) 
  
  fcst_Dholt <- holt(train, h=26, damped = TRUE) #14
  ...
  fcst_snaive <- snaive(train, h=26)     #22
...
test2 <- subset(raw_data2, start = (length(raw_data2)-25))

fit_tbats2 <- tbats(train2)    #1
fcst_tbats2 <- forecast(fit_tbats2, h=26)
error_tbats2 <- smape_cal(fcst_tbats2$mean,test2)



fcst_stla2 <- try(stlf(train2, h=26,forecastfunction = thetaf),silent = TRUE)    #4
error_stla2 <- tryCatch(error_stla2<- smape_cal(fcst_stla2$mean,test2) ,
                        error = function(e) {return(Inf)})                     

fcst_stle2 <- try(stlf(train2, h=26, method = "ets"),silent = TRUE)      #5
error_stle2 <- tryCatch(error_stle2<- smape_cal(fcst_stle2$mean,test2) ,
                        error = function(e) {return(Inf)})   


fcst_stlar <- try(stlf(train2, h=26, method = "arima"),silent = TRUE)      #5
...
if (pqv <= lvq && pqv < 10) {
if(pq == 1) {
              fcst <- forecast(tbats(raw_data), h=13)
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==2){
              ...
              fcst <- forecast(bestfit3, xreg = fourier(raw_data, K=kk, h=13))
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==3){
              ...
              kk=which.min(aic_model2)
              bestfit3 <- auto.arima(raw_data, xreg=fourier(raw_data, K=kk), seasonal=FALSE)
              fcst1 <- forecast(bestfit3, xreg = fourier(raw_data, K=kk, h=13)) 
              fcst2 <- forecast(tbats(raw_data), h=13)
              fcstm <- (fcst1$mean+fcst2$mean)/2
              fcstu <- (fcst1$upper[,2]+fcst2$upper[,2])/2
              fcstl <- (fcst1$lower[,2]+fcst2$lower[,2])/2
            } else if (pq==4){
              fcst <- stlf(raw_data, h=13, forecastfunction=thetaf)
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==5){
              fcst <- stlf(raw_data, h=13, method = "ets")
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==6){
              fcst <- forecast(nnetar(raw_data), PI=TRUE, h=13)
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==7){
              fcst <- thetaf(raw_data, h=13)
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==8){
              fcstm <- rep(mean(raw_data[(length(raw_data)-13):length(raw_data)]),13)
              fcstu <- naive(raw_data, h=13)$upper[,2] 
              fcstl <- naive(raw_data, h=13)$lower[,2] 
            } else if (pq==9){
              fcst <- naive(raw_data, h=13)
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==10){
              fcst <- ses(raw_data, h=13, alpha = .3)
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==11){
              fcst <- ses(raw_data, h=13, alpha = .1)
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==12){
              fcst <- ses(raw_data, h=13)
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==13){
              fcst <- holt(raw_data, h=13)
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==14){
              fcst <- holt(raw_data, h=13, damped = TRUE)
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==15){
              fcstm <- (thetaf(raw_data, h=13)$mean +  snaive(raw_data, h=13)$mean + naive(raw_data, h=13)$mean+ses(raw_data, h=13, alpha = .3)$mean +  
                         ses(raw_data, h=13, alpha = .1)$mean + ses(raw_data, h=13)$mean + holt(raw_data, h=13)$mean + holt(raw_data, h=13, damped = TRUE)$mean)/8
              fcstu <- (thetaf(raw_data, h=13)$upper[,2] +  snaive(raw_data, h=13)$upper[,2] + naive(raw_data, h=13)$upper[,2]+ses(raw_data, h=13, alpha = .3)$upper[,2] +  
                ses(raw_data, h=13, alpha = .1)$upper[,2] + ses(raw_data, h=13)$upper[,2] + holt(raw_data, h=13)$upper[,2] + holt(raw_data, h=13, damped = TRUE)$upper[,2])/8
              fcstl <- (thetaf(raw_data, h=13)$lower[,2]+  snaive(raw_data, h=13)$lower[,2] + naive(raw_data, h=13)$lower[,2]+ses(raw_data, h=13, alpha = .3)$lower[,2] +  
                         ses(raw_data, h=13, alpha = .1)$lower[,2] + ses(raw_data, h=13)$lower[,2] + holt(raw_data, h=13)$lower[,2] + holt(raw_data, h=13, damped = TRUE)$lower[,2])/8
              ...
              bestfit3 <- auto.arima(raw_data, xreg=fourier(raw_data, K=kk), seasonal=FALSE)
              fcst1 <- forecast(bestfit3, xreg = fourier(raw_data, K=kk, h=13)) 
              fcst2 <- thetaf(raw_data, h=13)
              fcstm <- (fcst1$mean+fcst2$mean)/2
              fcstu <- (fcst1$upper[,2]+fcst2$upper[,2])/2
              fcstl <- (fcst1$lower[,2]+fcst2$lower[,2])/2
              
            } else if (pq==18){
              fcst1 <- forecast(tbats(raw_data), h=13)
              fcst2 <- thetaf(raw_data, h=13)
              fcstm <- (fcst1$mean+fcst2$mean)/2
              fcstu <- (fcst1$upper[,2]+fcst2$upper[,2])/2
              fcstl <- (fcst1$lower[,2]+fcst2$lower[,2])/2
            } else if (pq==19){
              fcst1 <- thetaf(raw_data, h=13)
              fcst2 <- ses(raw_data, h=13, alpha = .1)   
              fcstm <- (fcst1$mean + fcst2$mean)/2
              fcstu <- (fcst1$upper[,2] + fcst2$upper[,2])/2
              fcstl <- (fcst1$lower[,2] + fcst2$lower[,2])/2
            } else if (pq==20){
              fcst1 <- ses(raw_data, h=13, alpha = .3)
              fcst2 <- ses(raw_data, h=13, alpha = .1) 
              fcstm <- (fcst1$mean + fcst2$mean)/2
              fcstu <- (fcst1$upper[,2] + fcst2$upper[,2])/2
              fcstl <- (fcst1$lower[,2] + fcst2$lower[,2])/2
              ...
              fcst1 <- ses(raw_data, h=13, alpha = .3)
              fcst2 <- ses(raw_data, h=13, alpha = .1)
              fcst3 <- ses(raw_data, h=13)
              fcstm <- (fcst1$mean+fcst2$mean+fcst3$mean)/3
              fcstu <- (fcst1$upper[,2]+fcst2$upper[,2]+fcst3$upper[,2])/3
              fcstl <- (fcst1$lower[,2]+fcst2$lower[,2]+fcst3$lower[,2])/3
            } else if (pq==22){
              fcst <- snaive(raw_data, h=13)
              fcstm <- fcst$mean
              fcstu <- fcst$upper[,2]
              fcstl <- fcst$lower[,2]
            } else if (pq==23){
              fcst1 <- stlf(raw_data, h=13, forecastfunction=thetaf)
              fcst2 <- stlf(raw_data, h=13, method = "ets")   
              fcstm <- (fcst1$mean+fcst2$mean)/2
              fcstu <- (fcst1$upper[,2]+fcst2$upper[,2])/2
              fcstl <- (fcst1$lower[,2]+fcst2$lower[,2])/2
            } else if (pq==24){
              ...
              bestfit3 <- auto.arima(raw_data, xreg=fourier(raw_data, K=kk), seasonal=FALSE)
              fcst1 <- forecast(bestfit3, xreg = fourier(raw_data, K=kk, h=13)) 
              fcst2 <- stlf(raw_data, h=13, forecastfunction=thetaf)
              fcst3 <- stlf(raw_data, h=13, method = "ets")
              fcst4 <- forecast(tbats(raw_data), h=13)
              fcst5 <- thetaf(raw_data, h=13)
              ...
              fcst1 <- forecast(nnetar(raw_data),PI=TRUE, h=13)
              fcst2 <- thetaf(raw_data, h=13)
              fcstm <- (fcst1$mean+fcst2$mean)/2
              fcstu <- (fcst1$upper[,2]+fcst2$upper[,2])/2
              fcstl <- (fcst1$lower[,2]+fcst2$lower[,2])/2
              ...
              fcst1 <- forecast(nnetar(raw_data),PI=TRUE, h=13)
              fcst2 <- thetaf(raw_data, h=13)
              fcst3 <- forecast(tbats(raw_data), h=13)
              fcstm <- (fcst1$mean+fcst2$mean+fcst3$mean)/3
              fcstu <- (fcst1$upper[,2]+fcst2$upper[,2]+fcst3$upper[,2])/3
              fcstl <- (fcst1$lower[,2]+fcst2$lower[,2]+fcst3$lower[,2])/3
            }
  
}else if (lvq < pqv && lvq <10) { 
  
  
  if(lv == 1) {
    fcst <- forecast(tbats(raw_data2), h=13)
    fcstm <- fcst$mean
    fcstu <- fcst$upper[,2]
    fcstl <- fcst$lower[,2]
  } else if (lv==2){
    fcst <- stlf(raw_data2, h=13, forecastfunction=thetaf)
    fcstm <- fcst$mean
    fcstu <- fcst$upper[,2]
    fcstl <- fcst$lower[,2]
  } else if (lv==3){
    fcst <- stlf(raw_data2, h=13, method = "ets")
    fcstm <- fcst$mean
    fcstu <- fcst$upper[,2]
    fcstl <- fcst$lower[,2]
  } else if (lv==4){
    fcst <- stlf(raw_data2, h=13, method = "arima")
    fcstm <- fcst$mean
    fcstu <- fcst$upper[,2]
    fcstl <- fcst$lower[,2]
  } 
}  else {
  a1<- mean(cross_validation(raw_data, method = 'theta', error = 'RMSE',step_ahead = 4, start = length(raw_data)-13)[4,], na.rm = TRUE)
  b1 <- mean(cross_validation(raw_data, method = 'naive', error = 'RMSE',step_ahead = 4, start = length(raw_data)-13)[4,], na.rm = TRUE)
  ifelse(length(raw_data)<250, c1 <- mean(cross_validation(raw_data, method = 'tbats', error = 'RMSE',step_ahead = 4, start = length(raw_data)-13)[4,], na.rm = TRUE), c1 <- Inf )
   d1 <- mean(cross_validation(raw_data, method = 'stla', error = 'RMSE',step_ahead = 4, start = length(raw_data)-13)[4,], na.rm = TRUE)
   e1 <- mean(cross_validation(raw_data, method = 'stle', error = 'RMSE',step_ahead = 4, start = length(raw_data)-13)[4,], na.rm = TRUE)
   f1 <- mean(cross_validation(raw_data, method = 'stlt', error = 'RMSE',step_ahead = 4, start = length(raw_data)-13)[4,], na.rm = TRUE)
   g1 <- mean(cross_validation(raw_data, method = 'snaive', error = 'RMSE',step_ahead = 4, start = length(raw_data)-13)[4,], na.rm = TRUE)
  
  
   
   gamma <- c(a1,b1,c1,d1,e1,f1,g1)
   lp <- which.min(as.matrix( gamma, nrow=7))
   if(lp == 1) {
     fcst <- thetaf(raw_data, h=13)
     fcstm <- fcst$mean
     fcstu <- fcst$upper[,2]
     ...
     fcst <- stlf(raw_data, h=13, method='arima')
     fcstm <- fcst$mean
     fcstu <- fcst$upper[,2]
     fcstl <- fcst$lower[,2]
   } else if (lp==5){
...
write.csv(fcstm,file = "output/weekly-v18.csv") 
write.csv(fcstu,file = "output/weekly-v18-upper.csv") 
write.csv(fcstl,file = "output/weekly-v18-lower.csv") 
